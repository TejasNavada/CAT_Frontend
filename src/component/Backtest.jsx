...
const Backtest = ({ portfolio, transactions, startHeight,efficientOrPretty }) => {
  ...
  useEffect(() => {
    ...
    // Create scales
    const x = d3.scaleUtc()
      .domain(d3.extent(portfolio, d => d.date))
      .range([margin.left, width - margin.right]);

    const y = d3.scaleLinear()
      .domain([0, d3.max(portfolio, d => d.total)])
      .range([height - margin.bottom, margin.top]);

      
    let currentX = x;
    const StockTooltip = d3.select("body")
    .append("div")
    .attr("class", "tooltip")
    ...
    const stockKeys = Object.keys(portfolio[0].stockValues);
    console.log(d3.schemeCategory10)
    const colorScale = d3.scaleOrdinal(d3.schemeTableau10);
    const transformedPortfolio = portfolio.map(d => ({
      date: d.date,
      cash: d.cash,
      ...d.stockValues // Spread stock values directly
    }));
    
    
    // Define the stack generator
    const stack = d3.stack()
    .keys(stockKeys) // Stack each stock
    .value((d, key) => d[key]) // Get stock values directly
    .order(d3.stackOrderNone)
    .offset(d3.stackOffsetNone);

    const stackedData = stack(transformedPortfolio);

    svg.selectAll(".stock-area")
    .data(stackedData)
    .enter()
    .append("path")
    .attr("class", d => `stock-area stock-area-${d.index}`)
    .attr("clip-path", "url(#chart-clip)")
    .attr("fill", d => colorScale(d.key))
    .attr("d", d3.area()
        .x(d => currentX(d.data.date)) // X-axis based on date
        .y0(d => y(d[0] + d.data.cash)) // Base at cash + previous stocks
        .y1(d => y(d[1] + d.data.cash)) // Top of stack
    )
    ...
    .on("mousemove", function (event, d) {
      ...
      const date = currentX.invert(adjustedX);
      const bisect = d3.bisector(d => d.date).left;
      let i = bisect(portfolio, date, 1);
      i = Math.max(1, Math.min(i, portfolio.length - 1));
      const d0 = portfolio[i - 1];
      const d1 = portfolio[i];
      if (!d0 || !d1) return;
      const dataPoint = date - d0.date > d1.date - date ? d1 : d0;
      //console.log(dataPoint)
      // Get the stock value at the closest date
      const stockValue = dataPoint.stockValues[d.key] || 0;
    ...
    chart.append('path')
      ...
      .on("mouseover", function (event) {
        StockTooltip.style("visibility", "visible")
            .text("Cash"); // Show "Cash" when hovered
        d3.select(this).style("opacity", 0.8);
      })
    ...
    transactionGroup.selectAll(".transaction-line")
      ...
      function zoomed(event) {
        ...
        svg.selectAll(".stock-area")
        .attr("d", d3.area()
            .x(d => currentX(d.data.date)) // Update x-position for zooming
            .y0(d => y(d[0] + d.data.cash)) // Adjust base of stack
            .y1(d => y(d[1] + d.data.cash)) // Adjust top of stack
        );
        // Update all stock areas
        //old code...
        // stockKeys.forEach((ticker, index) => {
        //   svg.select(`.stock-area-${index}`)
        //       .attr("d", d3.area()
        //           .x(d => currentX(d.date)) // Adjust x-position based on zoom
        //           .y0((d, i) => {
        //               let sumBelow = d.cash;
        //               for (let j = 0; j < index; j++) {
        //                   sumBelow += d.stockValues[stockKeys[j]];
        //               }
        //               return y(sumBelow);
        //           })
        //           .y1(d => {
    ...
    function update(date) {
      ...
      if (d?.date && d?.total) {
        ...
        .call(text => text
            .selectAll("tspan")
            .data([formatDate(d.date), formatValue(d.total)])
            .join("tspan")
            .attr("x", 0)
            .attr("y", (_, i) => `${i * 1.1}em`)
    ...
    .on("mousemove touchmove", function(event) {
      ...
      if (currentDate >= portfolio[0].date && currentDate <= portfolio[portfolio.length - 1].date) {
        update(currentDate);
      }
    });
